 ...
 */
function findItemById(id) {let foundItem = items.find((item) => item.id === id)
  return foundItem
}
/**
...
function calculateTotalInventory() {
 return items.reduce((total, item) => total + item.inventory, 
  0)
}
/**
...
function calculateAllInventoryPrice() {
  return items.reduce((total, item) => total + item.price *item.inventory,
  0)
 }
...
function logCartItems() {
  cart.forEach((itemId) => {
    let item = items.find((item) => item.id === item.id);
    if (item) {
...
function calculateTotalCartPrice() {
  ...
  /* I dont understand this quetion, even when looking at the solution code*/
}
